...
// Aba "Aluno" da página /retencao — saídas de ALUNO (era o conteúdo inteiro da
// página antes de 2026-08-10). Aqui "troca de professor" é o aluno que mudou de
// professor e segue matriculado; NÃO é o turnover de professor que a King mede
// — esse está na aba Professor.

export function RetencaoAlunoTab({ desde, ate }: { desde: string; ate: string }) {
  ...
  const totalTroca  = porGrupo.reduce((s, r) => s + r.turnover, 0)
  const semFlag     = totalSaidas - totalChurn - totalTroca
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-px rounded-2xl overflow-hidden bg-line-soft ring-1 ring-line-soft">
        ...
        <Tile valor={totalTroca}  rotulo="Trocaram de professor" sub={`${pct(totalTroca)}% · realocação`} />
            ...
            <table className="w-full text-[12.5px]">
              <thead>
                <tr className="text-[10.5px] uppercase tracking-[0.06em] text-ink-muted">
                  ...
                  <th className="text-right font-semibold py-2">Trocou de professor</th>
                </tr>
              </thead>
                ...
                <li key={m.motivo} className="space-y-1">
                  <div className="flex items-center justify-between gap-2 text-[12px]">
                    ...
                    <span className="tabular-nums text-ink-muted">
                      {m.total} <span className="text-ink-subtle">({m.churn} escola · {m.turnover} troca)</span>
                    </span>
                  </div>
